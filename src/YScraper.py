...
class App:
    ...
    # ----YOUTUBE-------
    def _ensure_comments_loaded(self, driver, max_retries=3):
        """Ensure comments are loaded with retry mechanism."""
        for attempt in range(max_retries):
            ...
            self._log_youtube_warning(f"Comments not found on attempt {attempt + 1}, retrying...")
            # Scroll to a comments section again and wait
            driver.execute_script(
                "document.querySelector('#comments').scrollIntoView({behavior: 'smooth', block: 'center'});")
            time.sleep(2)

        return False

    def _extract_replies(self, driver, comment_section, include_photos):
        """Extract replies for a comment with improved reliability."""
        replies = []

        # Check if there are replies and if they're expanded
        reply_buttons = comment_section.find_elements(
            By.CSS_SELECTOR,
            'ytd-button-renderer#more-replies, .more-button, .ytd-comment-replies-renderer'
        )

        # If there are reply buttons but replies aren't expanded, skip
        # The main _expand_replies method should handle this expansion
        if not reply_buttons:
            return replies

        # Look for reply elements with multiple possible selectors
        reply_elements = []
        for selector in [
            ...
            '.ytd-comment-thread-renderer .reply'
        ]:
            elements = comment_section.find_elements(By.CSS_SELECTOR, selector)
            if elements:
        ...
        for reply_elem in reply_elements:
            try:
                # Use the same improved JavaScript extraction for replies
                reply_data = driver.execute_script("""
                    function extractReplyData(reply) {
                        ...
                        const textSelectors = [
                            ...
                            '#comment-content',
                            'div#content yt-formatted-string'
                        ];
                        ...
                        const timeSelectors = [
                            ...
                            '.ytd-comment-renderer span.ytd-comment-published-time'
                        ];
                        ...
                        let replyId = "";
                        try {
                            const idAttribute = reply.id || "";
                            if (idAttribute.includes('comment-')) {
                                replyId = idAttribute.split('comment-')[1];
                            } else if (reply.getAttribute('data-comment-id')) {
                                replyId = reply.getAttribute('data-comment-id');
                            }
                        } catch (e) {
                            // Ignore ID extraction errors
                        }

                        return {author, text, time, likes, replyId};
                    }
                ...
                if include_photos:
                    try:
                        img_selectors = ['img#img', 'yt-img-shadow img', '#author-thumbnail img']
                        for selector in img_selectors:
                            try:
                                img_elem = reply_elem.find_element(By.CSS_SELECTOR, selector)
                                profile_photo = img_elem.get_attribute('src')
                                if profile_photo:
                                    reply_data['profile_photo'] = profile_photo
                                    break
                            except:
                                continue
    ...
    def _mute_youtube_audio(self, driver):
        """
        Mutes audio on YouTube videos by manipulating the video player via JavaScript.
        Should be called after a page with video content has loaded.
        """
        try:
            ...
            # Execute JavaScript to mute the video
            driver.execute_script("""
                // Method 1: Standard HTML5 video muting
                var videos = document.querySelectorAll('video');
                for(var i=0; i<videos.length; i++) {
                    videos[i].muted = true;
                }

                // Method 2: YouTube-specific muting
                var ytPlayer = document.querySelector('.html5-video-player');
                if(ytPlayer) {
                    ytPlayer.muted = true;
                    // Alternative YouTube-specific approach
                    if(ytPlayer.setMuted) {
                        ytPlayer.setMuted(true);
                    }
                }
            """)
            self._log_youtube_info("Audio muted successfully")
        except Exception as e:
            self._log_youtube_warning(f"Failed to mute audio: {e}")
    ...
    def setup_youtube_scraper_tab(self):
        ...
        ttk.Label(mode_selection_frame, text="Scrape Mode:", style="Bold.TLabel").pack(side=tk.TOP, anchor=tk.W,
                                                                                       pady=(0, 3))
        ...
        self.y_channel_limit_entry = ttk.Entry(self.y_channel_limit_frame, textvariable=self.y_channel_limit_var,
                                               width=10)
        ...
        self.y_list_browse_button = ttk.Button(self.y_list_file_frame, text="Browse...",
                                               command=self.browse_youtube_list_file)
        # --- End List File Input ---
        ...
        options_frame.pack(side=tk.LEFT, padx=15,
                           anchor=tk.NW)  # Pack to the left of input_frame, but after dynamic_input_area

        self.y_include_replies = tk.BooleanVar(value=False)
        ttk.Checkbutton(options_frame, text="Include Replies", variable=self.y_include_replies,
                        style="TCheckbutton").pack(anchor=tk.W, pady=(0, 2))

        self.y_include_photos = tk.BooleanVar(value=False)
        ttk.Checkbutton(options_frame, text="Include Profile Photos", variable=self.y_include_photos,
                        style="TCheckbutton").pack(anchor=tk.W)

        # --- Debug Mode Checkbox ---
        self.y_debug_mode_var = tk.BooleanVar(value=False)  # Default to False (headless)
        ttk.Checkbutton(options_frame, text="Debug Mode (Show Browser)", variable=self.y_debug_mode_var,
                        style="TCheckbutton").pack(anchor=tk.W)
        ...
        self.y_pause_button = ttk.Button(btn_frame, text="Pause", command=self.pause_youtube_scraping,
                                         state=tk.DISABLED)
    ...
    def setup_youtube_pretty_tab(self):
        ...
        ttk.Radiobutton(sort_radio_frame, text="Asc", variable=self.y_sort_direction, value="Ascending",
                        style="TRadiobutton").pack(side=tk.LEFT)
        ttk.Radiobutton(sort_radio_frame, text="Desc", variable=self.y_sort_direction, value="Descending",
                        style="TRadiobutton").pack(side=tk.LEFT)
        ...
        for i, (field, var) in enumerate(self.y_export_fields.items()):
            ttk.Checkbutton(fields_frame, text=field.capitalize(), variable=var, style="TCheckbutton").grid(row=0,
                                                                                                            column=i,
                                                                                                            padx=5)
        self.y_export_xml_button = ttk.Button(export_buttons_frame, text="Export XML",
                                              command=self.export_youtube_to_xml, state=tk.DISABLED)
        self.y_export_xml_button.pack(side=tk.LEFT, padx=2)
        self.y_export_json_button = ttk.Button(export_buttons_frame, text="Export JSON",
                                               command=self.export_youtube_to_json, state=tk.DISABLED)
        self.y_export_json_button.pack(side=tk.LEFT, padx=2)
        self.y_export_csv_button = ttk.Button(export_buttons_frame, text="Export CSV",
                                              command=self.export_youtube_to_csv, state=tk.DISABLED)
        ...
        ttk.Button(view_buttons_frame, text="Expand All", command=self.expand_all_youtube_replies).pack(side=tk.LEFT,
                                                                                                        padx=2)
        ttk.Button(view_buttons_frame, text="Collapse All", command=self.collapse_all_youtube_replies).pack(
            side=tk.LEFT, padx=2)
        ...
        self.y_tree = ttk.Treeview(tree_frame, columns=("author", "time", "likes", "text"), show="tree headings",
                                   style="Treeview")
    ...
    def setup_youtube_user_ops_tab(self):
        ...
        self.y_filter_author = ttk.Entry(lf);
        self.y_filter_author.pack(side=tk.LEFT, fill=tk.X, expand=True)  # Default TEntry
        ttk.Button(lf, text="Filter", command=self.filter_youtube_by_author).pack(side=tk.LEFT,
                                                                                  padx=5)  # Default TButton
        # Tree
    ...
    def youtube_scrape_thread(self, url, scroll_times, include_replies=False, include_photos=False):
        ...
        try:
            ...
            driver.get(url)
            self._mute_youtube_audio(driver)
    ...
    def _scroll_for_comments(self, driver, scroll_times, scrolling_weight):
        ...
        for attempt in range(5):  # Try a few times to find comments section
            ...
            try:
                ...
                if comments_container and comments_container.is_displayed():
                    ...
                    self.master.after(0, lambda: self.y_status.set(
                        "Comments section located. Proceeding to load comments."))
        ...
        if not comments_section_found:
            self.master.after(0, lambda: self.y_status.set(
                "Could not definitively locate comments section, attempting general scroll..."))
                ...
                while time.time() - wait_for_load_start_time < max_wait_for_load_time:
                    ...
                    try:
                        spinner = driver.find_element(By.CSS_SELECTOR,
                                                      "paper-spinner[active], tp-yt-paper-spinner[active]")
        ...
        try:
            ...
            self.master.after(0,
                              lambda: self.y_status.set("Final comment load wait timed out (spinner might persist)."))
            pass
    ...
    def _process_comments(self, driver, include_replies, include_photos, comments_processing_start, processing_weight):
        """Process comments and extract data with improved performance and reliability."""
        # Initial check with retry mechanism
        comments_loaded = self._ensure_comments_loaded(driver, max_retries=3)
        if not comments_loaded:
            self._log_youtube_warning("Failed to load comments after multiple attempts")
            return []
        ...
        batch_size = 20  # Process 20 comments at a time
        for batch_start in range(0, total_sections, batch_size):
            if self.scraping_stopped:
                return comments

            batch_end = min(batch_start + batch_size, total_sections)
            current_batch = comment_sections[batch_start:batch_end]

            # Update progress
            progress_value = comments_processing_start + ((batch_start / max(1, total_sections)) * processing_weight)
            self.master.after(0, lambda val=batch_start, tot=total_sections, end=batch_end:
            self.y_status.set(f"Processing comments... {val}-{end}/{tot}"))
            ...
            for section in current_batch:
                try:
                    # Extract comment data using improved JavaScript extraction
                    comment_data = driver.execute_script("""
                        function extractCommentData(section) {
                            ...
                            // Extract likes with multiple fallbacks
                            let likes = "0";
                            const likesSelectors = [
                                '#vote-count-middle', 
                                'span.like-count',
                            ...
                            // Extract comment ID if possible (for unique identification)
                            let commentId = "";
                            try {
                                const idAttribute = section.id || "";
                                if (idAttribute.includes('comment-')) {
                                    commentId = idAttribute.split('comment-')[1];
                                } else if (section.getAttribute('data-comment-id')) {
                                    commentId = section.getAttribute('data-comment-id');
                                }
                            } catch (e) {
                                // Ignore ID extraction errors
                            }

                            return {author, text, time, likes, commentId};
                        }
                    ...
                    if include_photos:
                        try:
                            ...
                            img_elem = None
                            for selector in ['img#img', 'yt-img-shadow img', '#author-thumbnail img']:
                                try:
                                    img_elem = section.find_element(By.CSS_SELECTOR, selector)
                                    if img_elem:
                                        break
                                except:
                                    continue
                            ...
                            print(f"Error fetching profile photo: {img_error}")
            ...
            # Process replies for the batch if requested
            if include_replies and batch_comments:
                for i, comment_data in enumerate(batch_comments):
                    if i < len(current_batch):  # Safety check
                        section = current_batch[i]
                        try:
                            # Process replies with improved handling
                            replies = self._extract_replies(driver, section, include_photos)
                            comment_data['replies'] = replies
                        except Exception as e:
                            print(f"Error processing replies: {e}")

            # Add batch comments to main comments list
            comments.extend(batch_comments)

            # Check if scraping was stopped or paused
            if self.scraping_stopped:
                return comments
            while self.scraping_paused and not self.scraping_stopped:
                time.sleep(0.5)
        ...
        try:
            ...
            for i, video_url in enumerate(url_list):
                ...
                try:
                    ...
                    try:
                        title_element = WebDriverWait(driver, 7).until(
                            EC.visibility_of_element_located((By.CSS_SELECTOR,
                                                              "h1.ytd-watch-metadata #video-title, yt-formatted-string.ytd-watch-metadata[slot='title']"))
                        )
                    ...
                    self.master.after(0, lambda
                        p=base_progress_for_this_video + progress_increment_for_this_video * 0.4: self.y_progress.configure(
                        value=p))
                    if self.scraping_stopped: continue
                    ...
                    if include_replies:
                        ...
                        self.master.after(0, lambda
                            p=base_progress_for_this_video + progress_increment_for_this_video * 0.7: self.y_progress.configure(
                            value=p))
                        if self.scraping_stopped: continue
                    ...
                    self.master.after(0, lambda
                        p=base_progress_for_this_video + progress_increment_for_this_video: self.y_progress.configure(
                        value=p))
            ...
            if not self.scraping_stopped:
                if all_comments_from_list:
                    ...
                    self.master.after(0, lambda: self.y_status.set(
                        "List scrape complete. No comments found in processed videos."))
                    self.master.after(0, lambda: messagebox.showinfo("List Scrape Complete",
                                                                     "No comments found in the processed videos from the list."))
            else:
                    ...
                    try:
                        ...
                        if 0 <= parent_idx < len(all_comments):
                            ...
                            if 'replies' in parent_comment_obj and 0 <= reply_idx < len(parent_comment_obj['replies']):
                                ...
                                print(
                                    f"Debug: Reply data not found. Parent index: {parent_idx}, Reply index: {reply_idx}, Parent has replies: {'replies' in parent_comment_obj}")
                        else:
                            print(
                                f"Debug: Parent index {parent_idx} out of bounds for all_comments (len {len(all_comments)})")
                    ...
                    if 'r' not in selected_id:
                        try:
                            ...
                            if 0 <= comment_idx < len(all_comments):
                                ...
                                print(
                                    f"Debug: Comment index {comment_idx} out of bounds for all_comments (len {len(all_comments)})")
                        except (ValueError, IndexError) as e:
                            print(f"Error parsing main comment ID ('{selected_id}'): {e}")
                    else:
                        print(
                            f"Warning: Item '{selected_id}' appears to be a reply (contains 'r') but has no parent_tree_id.")
    ...
    def download_profile_photos(self):
        ...
        if not has_photos:
            messagebox.showinfo("Info",
                                "No profile photos available. Try scraping with 'Include Profile Photos' option.")
            return
        ...
        def download_thread():
            ...
            # Done
            self.master.after(0,
                              lambda: self.y_status.set(f"Done! Downloaded {downloaded}/{total_photos} profile photos"))
            self.master.after(0, lambda: self.y_progress.configure(value=100))
    ...
    def show_youtube_context_menu(self, event):
            ...
            if 'r' in item_id:  # It's a reply
                ...
                if self.youtube_comments and 0 <= parent_idx < len(self.youtube_comments):
                    ...
                    if 'replies' in parent_comment and 0 <= reply_idx < len(parent_comment['replies']):
                        ...
                        self._log_youtube_warning(
                            f"Context menu: Reply index {reply_idx} out of bounds for parent c{parent_idx}.")
                else:
        ...
        menu.add_separator()
        menu.add_command(label="Copy Comment Text",
                         command=lambda cd=comment_data_obj: self._copy_comment_text_to_clipboard(cd))
        menu.add_command(label="Copy Author Name",
                         command=lambda cd=comment_data_obj: self._copy_author_to_clipboard(cd))
    ...
    def get_channel_video_links(self, driver, channel_url_videos_tab, video_limit):
        """
        Navigates to a YouTube channel's video page, scrolls to load videos,
        ...
        try:
            ...
            self.master.after(0, lambda: self.y_status.set(
                f"Timeout loading videos page contents: {channel_url_videos_tab}"))
        ...
        while True:
            ...
            if video_limit != 'all' and isinstance(video_limit, int):
                try:
                    ...
                    current_video_item_elements = driver.find_elements(By.CSS_SELECTOR,
                                                                       "ytd-rich-item-renderer, ytd-grid-video-renderer")
                    current_elements_count = len(current_video_item_elements)
                    self.master.after(0, lambda n=current_elements_count, lim=video_limit: self.y_status.set(
                        f"Scrolling... ({n} video items found, aiming for {lim if isinstance(lim, int) else 'all'})"))
                    if current_elements_count >= video_limit:
                        ...
                        self.master.after(0, lambda n=current_elements_count: self.y_status.set(
                            f"Potentially enough video items ({n}) loaded for limit. Stopping scroll."))
                        break
                except Exception as e:
            ...
            if scroll_attempts_no_change >= MAX_NO_CHANGE_SCROLLS:
                self.master.after(0, lambda: self.y_status.set(
                    "Reached end of scrollable content or no new content loaded after multiple attempts."))
                break
        ...
        try:
            ...
            if not video_link_elements:  # Fallback for potentially different grid structures
                video_link_elements.extend(
                    driver.find_elements(By.CSS_SELECTOR, "ytd-grid-video-renderer a#video-title-link"))
            ...
            self.master.after(0, lambda n=len(video_link_elements): self.y_status.set(
                f"Found {n} potential video link elements. Extracting URLs..."))
            ...
            for elem_idx, elem in enumerate(video_link_elements):
                ...
                if video_limit != 'all' and isinstance(video_limit, int) and len(ordered_video_links) >= video_limit:
                    self.master.after(0, lambda lim=video_limit: self.y_status.set(
                        f"Reached video limit of {lim} during collection."))
                    break
                    ...
                    if href and "watch?v=" in href:  # Basic validation for a YouTube video link
                        ...
                        if normalized_href not in collected_urls_set:
                            ...
                            if elem_idx % 20 == 0:  # Update status periodically
                                self.master.after(0, lambda c=len(ordered_video_links): self.y_status.set(
                                    f"Collected {c} unique video links..."))
        ...
        if self.scraping_stopped:
            self.master.after(0, lambda c=len(final_links): self.y_status.set(
                f"Link collection stopped by user. {c} links gathered."))
        else:
            self.master.after(0, lambda c=len(final_links): self.y_status.set(
                f"Finished getting video links. Total collected: {c}"))
        ...
        try:
            ...
            if not video_urls:
                if not self.scraping_stopped:
                    ...
                    self.master.after(0, lambda: messagebox.showinfo("Channel Scrape Info",
                                                                     "No video links found or process stopped."))
                if driver:
                    driver.quit()
                self.master.after(0, self._reset_scraping_buttons)
                return
            ...
            if not self.scraping_stopped:
                if all_comments_from_channel:
                    ...
                    self.master.after(0, lambda: self.y_status.set(
                        "Channel scrape complete. No comments found in processed videos."))
                    self.master.after(0, lambda: messagebox.showinfo("Channel Scrape Complete",
                                                                     "No comments were found in the processed videos for this channel."))
            else:
    ...
    def get_channel_video_links(self, driver, channel_url_videos_tab, video_limit):
        ...
        try:
            ...
            if not video_link_elements:
                self.master.after(0, lambda: self.y_status.set(
                    "Primary/secondary item link selectors failed, trying broad 'a#video-title'."))
                video_link_elements = driver.find_elements(By.CSS_SELECTOR, "a#video-title")
                if video_link_elements:
                    self.master.after(0,
                                      lambda: self.y_status.set("Using broad fallback link selector (a#video-title)."))
            ...
            for elem_idx, elem in enumerate(video_link_elements):
                ...
                try:
                    ...
                    print(
                        f"Error extracting href from element or normalizing: {e_extract} on element: {elem.get_attribute('outerHTML')[:100]}")
                    continue
    ...
    def _log_youtube_info(self, msg):
        print(f"[INFO ] {msg}")

    def _log_youtube_warning(self, msg):
        print(f"[WARN ] {msg}")

    def _log_youtube_error(self, msg):
        print(f"[ERROR] {msg}")

    def _log_youtube_success(self, msg):
        print(f"[ OK  ] {msg}")
